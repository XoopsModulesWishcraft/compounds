...
if( $txn_id )
{
        $sql = "SELECT * FROM ".$GLOBALS['xoopsDB']->prefix("comp_transactions")." WHERE txn_id = '$txn_id'";
...
while (!$dbg && !$ERR && !feof($fp)) 
{
        ...
        if (strcmp ($res, "VERIFIED") == 0)
        {
                ...
                if( strcmp($payment_status, "Refunded") == 0)
                {
						...
						list($uid) = $GLOBALS['xoopsDB']->fetchRow($GLOBALS['xoopsDB']->queryF('SELECT uid FROM ' . $GLOBALS['xoopsDB']->prefix('users'). ' WHERE uname = "'.$_POST['custom'].'"'));
						if ($uid>0) 
							$GLOBALS['xoopsDB']->queryF('UPDATE ' . $GLOBALS['xoopsDB']->prefix('uid_link'). ' SET score = ' . $xoConfig['score'] .  ', donations = donations - 1, amount = amount - (' . (floatval($_POST['mc_gross']) - floatval($_POST['mc_fee'])) . ') WHERE uid = ' . $uid);							        
                        break;
                ...
                if( (strcmp($payment_status, "Completed") == 0) && ((strcmp($txn_type, "web_accept")== 0) || (strcmp($txn_type, "send_money")== 0)) )
                {
						...
						list($uid) = $GLOBALS['xoopsDB']->fetchRow($GLOBALS['xoopsDB']->queryF('SELECT uid FROM ' . $GLOBALS['xoopsDB']->prefix('users'). ' WHERE uname = "'.$_POST['custom'].'"'));
						if ($uid>0) 
							$GLOBALS['xoopsDB']->queryF('UPDATE ' . $GLOBALS['xoopsDB']->prefix('uid_link'). ' SET score = 0, donations = donations + 1, amount = amount + (' . (floatval($_POST['mc_gross']) - floatval($_POST['mc_fee'])) . ') WHERE uid = ' . $uid);
...
if( $dbg )
{
        $sql = "SELECT * FROM ".$GLOBALS['xoopsDB']->prefix("comp_transactions")." LIMIT 10";
...
if( $log )
{
        ...
        $sql = "INSERT INTO ".$GLOBALS['xoopsDB']->prefix("comp_translog")." VALUES (NULL,'" . strftime('%Y-%m-%d %H:%M:%S',mktime()) . "', '"
        ...
        {
                $sql =  "DELETE FROM ".$GLOBALS['xoopsDB']->prefix("comp_translog")." WHERE id < '" . $lowid . "'";
                $Result1 = $GLOBALS['xoopsDB']->queryF($sql);
        }
}
