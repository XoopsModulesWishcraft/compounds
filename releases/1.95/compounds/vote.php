<?php
	...
	if ($voteDataHandler->getCount($criteria)) {
		redirect_header('info.php?id='.$id, 4, sprintf(_CMP_VOTE_ALREADYTAKEN, $compoundHandler->renderSymbolisation($compound->getVar('symbol')),$session['ip']));
		exit(0);
	}
	...
	case 'vote':
		...
		if ($session['ip'] == $_POST['ip'] &&  $session['addy'] == gethostbyaddr($_SERVER['REMOTE_ADDR']))
		{
			...
			redirect_header('info.php?id='.$id, 4, sprintf(_CMP_VOTE_TAKEN, $compoundHandler->renderSymbolisation($compound->getVar('symbol')),$session['ip']));
		} else {
			redirect_header('info.php?id='.$id, 4, sprintf(_CMP_VOTE_UNMATCHED, $compoundHandler->renderSymbolisation($compound->getVar('symbol')),$session['ip']));
		}
		exit(0);
