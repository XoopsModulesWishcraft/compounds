	...
	case 'delete':
		...
		case "composites":
			...
			if (!is_object($composite[0])) {
				redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=composite', 4, _CMP_AM_ITEM_DOESNT_EXIST);					
				exit(0);
			}
			
			if (!isset($_POST['confirm'])) {
				xoops_cp_header();
				xoops_confirm(array('confirm'=>true, 'op'=>$_REQUEST['op'], 'fct'=>$_REQUEST['fct'],  'id'=>$_REQUEST['id']), $_SERVER['REQUEST_URI'], sprintf(_CMP_FRM_COMPOSITE_DELETE, $composite[0]->getVar('symbol')));
				xoops_cp_footer();
				exit(0);
			}
			...
			redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=composite', 4, sprintf(_CMP_AM_COMPOSITE_DELETED, $composite[0]->getVar('symbol')));					
			break;
		case "numberof":
			...
			if (!is_object($numberof[0])) {
				redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=numberof', 4, _CMP_AM_ITEM_DOESNT_EXIST);					
				exit(0);
			}
			
			if (!isset($_POST['confirm'])) {
				xoops_cp_header();
				xoops_confirm(array('confirm'=>true, 'op'=>$_REQUEST['op'], 'fct'=>$_REQUEST['fct'],  'id'=>$_REQUEST['id']), $_SERVER['REQUEST_URI'], sprintf(_CMP_FRM_NUMBEROF_DELETE, $numberof[0]->getVar('symbol')));
				xoops_cp_footer();
				exit(0);
			}
			...
			redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=numberof', 4, sprintf(_CMP_AM_NUMBEROF_DELETED, $numberof[0]->getVar('symbol')));					
			break;
		case "periodical":
			$periodicalHandler =& xoops_getmodulehandler('periodical', 'compounds');		
			...
			if (!is_object($periodical[0])) {
				redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=periodical', 4, _CMP_AM_ITEM_DOESNT_EXIST);					
				exit(0);
			}
			
			if (!isset($_POST['confirm'])) {
				xoops_cp_header();
				xoops_confirm(array('confirm'=>true, 'op'=>$_REQUEST['op'], 'fct'=>$_REQUEST['fct'],  'id'=>$_REQUEST['id']), $_SERVER['REQUEST_URI'], sprintf(_CMP_FRM_PERIODICAL_DELETE, $periodical[0]->getVar('symbol')));
				xoops_cp_footer();
				exit(0);
			}
			...
			redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=periodical', 4, sprintf(_CMP_AM_PERIODICAL_DELETED, $periodical[0]->getVar('symbol')));					
			break;
		default:
			...
			if ($GLOBALS['xoopsDB']->queryF($sql[0]))
				if ($GLOBALS['xoopsDB']->queryF($sql[1]))
					if ($GLOBALS['xoopsDB']->queryF($sql[2])) {
						...
						redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=list', 4, sprintf(_CMP_AM_COMPOUNDDELETE_SUCCESSFUL, $compoundHandler->renderSymbolisation($compound[0]->getVar('symbol'))));
					} else
						redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=list', 4, sprintf(_CMP_AM_COMPOUNDDELETE_UNSUCCESSFUL, $compoundHandler->renderSymbolisation($compound[0]->getVar('symbol'))));
				else
					redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=list', 4, sprintf(_CMP_AM_COMPOUNDDELETE_UNSUCCESSFUL, $compoundHandler->renderSymbolisation($compound[0]->getVar('symbol'))));
			else
				redirect_header(XOOPS_URL.'/modules/compounds/admin/index.php?op=list', 4, sprintf(_CMP_AM_COMPOUNDDELETE_UNSUCCESSFUL, $compoundHandler->renderSymbolisation($compound[0]->getVar('symbol'))));					
	...
	case 'save':
		...
		case "edit":	
			if ($_REQUEST['hyposise_editor_current'] != $_REQUEST['hyposise_editor'] || $_REQUEST['process_editor_current'] != $_REQUEST['process_editor'] || $_REQUEST['synopsise_editor_current'] != $_REQUEST['synopsise_editor'] ) {
				...
				<div style="float:left; border-right:dotted; padding-left:10px; padding-right:10px;">'._CMP_AM_HEADER_USER_SUBMISSIONS.$userSubmissions['url'].'</div>
				<div style="float:left; border-right: dotted; padding-left:10px; padding-right:10px;">'._CMP_AM_HEADER_MOLECULAR_SUBMISSIONS.$molecularSubmissions['url'].'</div>
				<div style="float:left; border-right: dotted; padding-left:10px; padding-right:10px;">'._CMP_AM_HEADER_7DAY_SUBMISSIONS.$sevendaySubmissions['url'].'</div>
				<div style="float:left; border-right: dotted; padding-left:10px; padding-right:10px;">'._CMP_AM_HEADER_DONATIONS_SOFAR.$donations['total_done'].'</div>
				<div style="float:left; border-right: dotted; padding-left:10px; padding-right:10px;">'._CMP_AM_HEADER_TOTAL_CASH.$donations['total_cash'].'</div>
			</div>';
